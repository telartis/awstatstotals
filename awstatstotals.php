...
$rs = mysqli_query($sql);
if ($rs) while ($row = mysqli_fetch_array($rs))
 ...
 * @return string
 */
function get_config(string $file): string
{
    $r = '';
 ...
 * @return array
 */
function read_history(string $file): array
{
 ...
 * @return array
 */
function parse_dir(string $dir): array
{
 ...
 * @return string
 */
function detect_language(string $DirLang): string
{
    $Lang = '';
 ...
 * @return array
 */
function read_language_data(string $file): array
{
    $r = [];
    if (file_exists($file)) {
...
<form action="<?php echo filter_var($_SERVER['SCRIPT_URL'], FILTER_SANITIZE_STRING); ?>">
